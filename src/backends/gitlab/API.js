...
export default class API {
  ...
  withAuthorizationHeaders = req =>
    reqrep.withHeaders(this.token ? { Authorization: `Bearer ${ this.token }` } : {}, req);

  buildRequest = req => flow([
    reqrep.withRoot(this.api_root),
    this.withAuthorizationHeaders,
    reqrep.withTimestamp,
  ])(req);

  request = async req => flow([this.buildRequest, reqrep.performRequest])(req);
  requestURL = url => flow([reqrep.fromURL, this.request])(url);
  ...
  fileDownloadURL = (path, ref=this.branch) => reqrep.toURL(this.buildRequest({
    url: `${ this.repoURL }/repository/files/${ encodeURIComponent(path) }/raw`,
    params: { ref },
  }));
  ...
  getCursorFromHeaders = headers => {
    ...
    const links = linksRaw.split(",")
      ...
      .reduce((acc, [link, key]) => Object.assign(acc, { [key]: reqrep.fromURL(link).toJS() }), {});
    return {
      actions: [
  ...
  // request
  fetchRequestCursor = flow([reqrep.withMethod("HEAD"), this.request, reqrep.then(this.getCursor)]);
  fetchCursorAndEntries = flow([
    reqrep.withMethod("GET"),
    this.request,
    p => Promise.all([p.then(x => console.log(x) || x).then(this.getCursor), p.then(this.responseToJSON)]),
    reqrep.then(x => { console.log(x); return x }),
    reqrep.then(([cursor, entries]) => ({ cursor, entries })),
  ]);
  fetchRelativeCursor = async (cursor, action) => fetchRequestCursor(cursor.data.links[action]);
  ...
  listFiles = async path => {
    const firstPageCursor = await this.fetchRequestCursor({
  ...
  deleteFile = (path, commit_message, options = {}) => {
    ...
    return flow([
      reqrep.fromURL,
      reqrep.withMethod("DELETE"),
      reqrep.withParams({ commit_message, branch }),
      this.requestJSON,
